...
@section('page-title', 'إضافة حالة جديدة')

@section('content')
<form id="caseForm" method="POST" action="{{ route('employee.cases.store') }}">
    @csrf
                ...
                <table id="testsTable">
                    ...
                    <tbody>
                        @foreach($tests as $test)
                            <tr class="test-row" data-category="{{ $test->category }}" data-name="{{ strtolower($test->name_ar . ' ' . $test->name_en) }}">
                                <td>
                                    <input type="checkbox" class="test-checkbox" value="{{ $test->id }}" onchange="toggleTest(this, {{ $test->id }}, '{{ addslashes($test->name_ar) }}', {{ $test->price }})">
                                </td>
                                <td>{{ $test->name_ar }}<br><small style="color:var(--text-secondary)">{{ $test->name_en }}</small></td>
                                <td>{{ $test->category }}</td>
...
async function submitCase(newCase) {
    ...
    try {
        const response = await fetch('{{ route("employee.cases.store") }}', {
            method: 'POST',
            headers: {
        ...
        if (result.success) {
            if (newCase) {
                ...
                window.location.href = '{{ route("employee.cases.index") }}';
            }
        } else {
            if (result.errors) {
                let msgs = [];
